...
const getEncryptionKey = (secret: string) => {
  if (!/^[a-fA-F0-9]{64}$/.test(secret)) {
    throw new Error('SMS_SETTINGS_SECRET must be a 64-character hexadecimal value.');
  }
  return Buffer.from(secret, 'hex');
};

export const encryptSmsCredential = (value: string, secret: string) => {
...
export const decryptSmsCredential = (encryptedValue: string, secret: string) => {
  try {
    ...
    if (error instanceof Error && error.message.includes('SMS_SETTINGS_SECRET')) throw error;
    throw new Error('Unable to decrypt SMS credential.');
  }
};
