...
exports.getPartnerAnalyticsOverview = void 0;
...
const formatMonthLabel = (date) => date.toLocaleDateString('en-IN', {
    month: 'short',
    year: '2-digit',
});
const getPartnerAnalyticsOverview = async (req, res, next) => {
    try {
        ...
        const liveListings = listings.filter((listing) => ['PUBLISHED', 'RESERVED', 'PAUSED'].includes(listing.status));
        const publishedListings = listings.filter((listing) => listing.status === 'PUBLISHED');
        const pendingListings = listings.filter((listing) => ['DRAFT', 'PENDING_APPROVAL', 'CHANGES_REQUESTED'].includes(listing.status));
        const soldListings = listings.filter((listing) => listing.status === 'SOLD');
        const wonLeads = leads.filter((lead) => lead.status === 'WON').length;
        const activeLeads = leads.filter((lead) => ['NEW', 'CONTACTED', 'INTERESTED', 'INSPECTION_SCHEDULED'].includes(lead.status)).length;
        const liveInventoryValue = liveListings.reduce((sum, listing) => sum + Number(listing.price || 0), 0);
        ...
        const topListings = listings
            ...
            wonLeadCount: listing.leads.filter((lead) => lead.status === 'WON').length,
            ...
            .sort((first, second) => second.leadCount - first.leadCount || second.price - first.price)
            .slice(0, 5);
        const now = new Date();
...
exports.getPartnerAnalyticsOverview = getPartnerAnalyticsOverview;
