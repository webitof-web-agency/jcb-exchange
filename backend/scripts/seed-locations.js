...
async function chunkedInsert(model, data, chunkSize = 1000) {
...
async function main() {
    ...
    const countriesPath = path_1.default.join(rootDir, 'nsc.countries.json');
    const statesPath = path_1.default.join(rootDir, 'nsc.states.json');
    const citiesPath = path_1.default.join(rootDir, 'nsc.cities.json');
    console.log('Reading countries...');
    const rawCountries = JSON.parse(fs_1.default.readFileSync(countriesPath, 'utf8'));
    ...
    const rawStates = JSON.parse(fs_1.default.readFileSync(statesPath, 'utf8'));
    const mappedStates = rawStates.map((s) => ({
        id: s._id,
        name: String(s.name),
    ...
    const rawCities = JSON.parse(fs_1.default.readFileSync(citiesPath, 'utf8'));
    ...
    await chunkedInsert(prisma_1.default.country, mappedCountries);
    console.log('Seeding states...');
    await chunkedInsert(prisma_1.default.state, mappedStates);
    console.log('Seeding cities... (This will take a few minutes)');
    await chunkedInsert(prisma_1.default.city, mappedCities);
