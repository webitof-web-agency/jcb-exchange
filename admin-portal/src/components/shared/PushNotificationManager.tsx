...
export default function PushNotificationManager() {
  ...
  const promptTitle = isPartner ? 'Stay Updated!' : 'Admin Alerts!';
  const promptMessage = isPartner
    ? 'Get instant alerts when your listing status changes or a new lead needs attention.'
    : 'Get instant push notifications for new partner signups, KYC approvals, and listings.';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-start space-x-4 bg-white/70 backdrop-blur-xl p-5 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/50 max-w-sm animate-in slide-in-from-bottom-5 duration-500 fade-in">
      ...
      <div className="flex-1 pr-2">
        <h3 className="text-gray-900 font-bold text-sm tracking-tight mb-1">{promptTitle}</h3>
