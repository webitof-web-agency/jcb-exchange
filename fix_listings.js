...
const path = "g:\\Webitof company\\jcbexchange\\admin-portal\\src\\app\\(partner)\\partner\\listings\\page.tsx";
let content = fs.readFileSync(path, 'utf-8');

const correctBlock = `      setListings((current) =>
...
// The broken code in partner looks like:
//      setListings((current) =>
//        current.map((item) => (item.id === listingId ? response.data.listing : item))
//      );
//
...
const pattern = /      setListings\(\(current\) =>\r?\n        current\.map\(\(item\) => \(item\.id === listingId \? response\.data\.listing : item\)\)\r?\n      \);\r?\n      \r?\n      setMessage\(response\.data\.message \|\| 'Availability updated successfully\.'\);\r?\n      setTimeout\(\(\) => setMessage\(''\), 3000\);[\s\S]*?Loading your listings\.\.\.<\/div>/;

const newContent = content.replace(pattern, correctBlock);
