...
# Replace from `setListings((current) =>` to `Loading your listings...</div>`
# In the current broken file it is:
#      setListings((current) =>
#        current.map((item) => (item.id === listingId ? response.data.listing : item))
#      );
...
new_content = pattern.sub(correct_block + "\n        ) : filteredListings.length === 0 ? (", content)
