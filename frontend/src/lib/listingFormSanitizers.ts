...
const textOnlyFields = new Set(['district', 'buyerName']);
const mixedTextFields = new Set([
  'title', 'brand', 'brandName', 'model', 'modelName', 'variant', 'grossPower',
  'nearbyLandmark', 'locationState', 'locationCity', 'state', 'city', 'buyerState', 'buyerCity',
]);
const multilineFields = new Set(['overview', 'description', 'additionalDescription']);
const uppercaseCodeFields = new Set(['registrationNo', 'chassisOrSerialNo']);

export const sanitizeListingFieldValue = (field: string, value: string) => {
  if (field === 'address') return sanitizeMixedText(value, 240);
...
export const sanitizeListingFormData = <T extends Record<string, unknown>>(form: T): T =>
  Object.fromEntries(
    Object.entries(form).map(([key, value]) => [
      key,
      typeof value === 'string' ? sanitizeListingFieldValue(key, value) : value,
    ]),
  ) as T;
