...
export const emptyListingRtoForm: ListingRtoFormState = {
  hirePurchaseStatus: 'PENDING', taxStatus: 'VALID', taxValidUntil: '', fitnessStatus: 'VALID', fitnessValidUntil: '',
  insuranceStatus: 'VALID', pucStatus: 'VALID', pucValidUntil: '', hsrpStatus: 'NO', rtoOffice: '', rtoAgentName: '',
  rtoExpenses: '', vehicleMaintenanceCost: '',
};

const sanitizeDigits = (value: string) => value.replace(/\D/g, '');
const sanitizeUppercase = (value: string) => value.toUpperCase().trimStart();

export const sanitizeListingRtoForm = (form: ListingRtoFormState): ListingRtoFormState => ({
  ...form,
  taxValidUntil: form.taxValidUntil.trim(), fitnessValidUntil: form.fitnessValidUntil.trim(),
  pucValidUntil: form.pucValidUntil.trim(), rtoOffice: sanitizeUppercase(form.rtoOffice),
  rtoAgentName: sanitizeUppercase(form.rtoAgentName), rtoExpenses: sanitizeDigits(form.rtoExpenses),
  vehicleMaintenanceCost: sanitizeDigits(form.vehicleMaintenanceCost),
});
...
export const validateListingRtoForm = (
  ...
  const validityChecks: Array<[string, string, string]> = [
    ['Tax Validity', form.taxStatus, form.taxValidUntil], ['Fitness Validity', form.fitnessStatus, form.fitnessValidUntil],
    ['Insurance Validity', form.insuranceStatus, base.insuranceExpiry], ['PUC Validity', form.pucStatus, form.pucValidUntil],
  ];
...
export const buildListingRtoDetails = (
  form: ListingRtoFormState,
  base: { registrationNo: string; vehicleType: string; vehicleModel: string; operatingHours: string; insuranceExpiry: string },
) => ({ ...form, registrationNo: base.registrationNo, vehicleType: base.vehicleType, vehicleModel: base.vehicleModel, operatingHours: base.operatingHours, insuranceValidUntil: base.insuranceExpiry });
